...
ARG BUILDAH_VERSION=main
WORKDIR /tmp/buildah
RUN apt-get update && apt-get -y install --no-install-recommends git ca-certificates wget && \
    git clone --config advice.detachedHead=false --depth 1 --branch "${BUILDAH_VERSION}" https://github.com/containers/buildah .
...
ENV CFLAGS='-static -pthread' \
    ...
    CGO_ENABLED=1
RUN echo IyEvYmluL3NoCgpBUkNIPSQoY2FzZSAkKHVuYW1lIC1tKSBpbgphbWQ2NCB8IHg4Nl82NCkKICBlY2hvIGFtZDY0CiAgOzsKYXJtNjQgfCBhYXJjaDY0KQogIGVjaG8gYXJtNjQKICA7Owplc2FjKQoKd2dldCAtcU9nby50Z3ogImh0dHBzOi8vZ28uZGV2L2RsL2dvJHsxOi0xLjIxLjl9LmxpbnV4LSRBUkNILnRhci5neiIK | base64 -d | sh -s -- 1.22.12 && \
