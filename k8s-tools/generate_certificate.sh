...
usage() {
  cat <<EOF
Generate certificate suitable for use with any Kubernetes Mutating/Validating Webhook.
usage: ${0} [OPTIONS]
...
The following flags are optional.
    --webhook-kind     Webhook kind, either MutatingWebhookConfiguration or ValidatingWebhookConfiguration (defaults to MutatingWebhookConfiguration)
EOF
}
...
if [ -s "/etc/kubernetes/pki/ca.crt" ] && [ -s "/etc/kubernetes/pki/ca.key" ]; then
  $(which cp) /etc/kubernetes/pki/ca.crt "$CA" >/dev/null 2>&1 || true
  $(which cp) /etc/kubernetes/pki/ca.key "$CAkey" >/dev/null 2>&1 || true
fi
[ -s "$CAkey" ] || openssl genrsa -out "$CAkey" 4096 >/dev/null
[ -s "$SERVER.key" ] || openssl genrsa -out "$SERVER.key" 4096 >/dev/null
cat <<EOF >"$SERVER.conf"
[req]
...
for webhook_index in $(seq "$(kubectl get "${kind:-mutatingwebhookconfiguration}" "${webhook}" -o template='{{.webhooks|len}}')"); do
  while true; do
    echo "INFO: Trying to patch webhook($kind: $webhook) adding the caBundle."
    if kubectl patch "${kind:-mutatingwebhookconfiguration}" "${webhook}" --type='json' -p "[{'op': 'replace', 'path': '/webhooks/$((webhook_index - 1))/clientConfig/caBundle', 'value':'${caBundle}'}]"; then
      break
    fi
...
# Patch the CustomResourceDefinition adding the caBundle.
for crd in $(kubectl get "${kind:-mutatingwebhookconfiguration}" "$webhook" -o template='{{range $wb:=.webhooks}}{{range $rule:=$wb.rules}}{{range $res:=$rule.resources}}{{range $apiG:=$rule.apiGroups}}{{$res}}.{{$apiG}}{{"\n"}}{{end}}{{end}}{{end}}{{end}}' | sort | uniq); do
  if [ "$(kubectl get crd "$crd" -o template='{{.spec.conversion.webhook.clientConfig.service.name}}')" = "$service" ]; then
    while true; do
      echo "INFO: Trying to patch CustomResourceDefinition($kind: $webhook) adding the caBundle."
      if kubectl patch crd "$crd" --type='json' -p "[{'op': 'replace', 'path': '/spec/conversion/webhook/clientConfig/caBundle', 'value':'${caBundle}'}]"; then
        break
      fi
      echo "INFO: CustomResourceDefinition($crd) not patched. Retrying in 5s..."
      sleep 5
    done
  fi
done
